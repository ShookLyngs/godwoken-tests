 ...
 * These are ERC20 token addresses on Godwoken testnet_v1:
    ...
    GWK: 0x2275Afe815dE66BeAbE7A2C03005537AB843afB2
...
describe("sUDT-ERC20 Proxy Contract", () => {
  ...
  it("Check the infos of the sUDT-ERC20 Proxy Contracts", async function () {
    // console.log(`symbol \t| name \t\t| decimals \t| totalSupply`);

    for (const id in proxyContracts) {
      const addr = proxyContracts[id];
      const contract = await ethers.getContractAt(abi, addr);

      const [sudtId, name, symbol, decimals] = await Promise.all([
        contract.sudtId(),
        contract.name(),
        contract.symbol(),
        contract.decimals(),
        // TODO: contract.totalSupply()
      ]);
      console.log({symbol, name, symbol, decimals, addr});
      // console.log(`${symbol} \t| ${name} \t\t| ${decimals} \t| ${totalSupply}`);
  ...
  it("Check TotalSupply of sUDT", async () => {
    const ckbProxy = await ethers.getContractAt(abi, proxyContracts["1"]);
    const totalSupply = await ckbProxy.totalSupply();

    // console.log(await ckbProxy.sudtId());
    console.log(totalSupply);

    // expect(totalSupply).equals(1);
  });
});
